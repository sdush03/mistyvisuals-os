...
contextBridge.exposeInMainWorld('api', {
  ...
  selectVideoFile: (title) => ipcRenderer.invoke('select-video-file', title),
  inspectCoverImage: (filePath) => ipcRenderer.invoke('inspect-cover-image', filePath),
  saveTempBakedCover: (config) => ipcRenderer.invoke('save-temp-baked-cover', config),
  processPhotos: (config) => ipcRenderer.invoke('process-photos', config),
  attachVideoToFilm: (config) => ipcRenderer.invoke('attach-video-to-film', config),
  onAttachVideoProgress: (callback) => ipcRenderer.on('attach-video-progress', (event, data) => callback(data)),

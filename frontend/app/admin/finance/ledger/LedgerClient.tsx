...
export default function FinanceLedgerPage() {
  ...
  const loadTransactions = async () => {
    ...
    try {
      ...
      if (referenceQuery.trim()) params.set('reference_code', referenceQuery.trim())
  ...
  }, [viewMode, viewLeadId, viewSourceId, referenceQuery, showDeleted, includeTransfers, mainTab, totalsMode])
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          <h2 className="text-2xl md:text-3xl font-semibold">Ledger</h2>
          <p className="text-sm text-neutral-600 mt-1">Track money in and money out with explicit entries.</p>
        </div>
        ...
        <section className={cardClass}>
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <div className="text-xs text-neutral-500">View and edit historical entries.</div>
            </div>
            <div className="flex flex-col gap-2 relative">
              ...
              <div className="flex items-center justify-end gap-2 text-xs">
                ...
                <label className="flex items-center gap-1.5 cursor-pointer">
                  ...
                  <span className="text-neutral-500">Include Transfers</span>
                </label>
              </div>
            </div>
          </div>
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            ...
            <div className="ml-auto text-sm text-neutral-600 flex items-center gap-4">
              ...
              {includeTransfers && <span className="text-xs text-neutral-500">Includes transfers</span>}
            </div>
          </div>
          ...
          <div className="mt-4 overflow-x-auto">
            <table className="min-w-full text-sm">
              ...
              <tbody className="divide-y divide-[var(--border)]">
                {loading && (
                  <tr>
                    <td className="px-4 py-4 text-sm text-neutral-500" colSpan={6}>Loading transactions…</td>
                  </tr>
                )}
                {!loading && transactions.length === 0 && (
                  <tr>
                    <td className="px-4 py-4 text-sm text-neutral-500" colSpan={6}>No transactions yet.</td>
                  </tr>
                )}
                {!loading && transactions.map(tx => {
                  if (editTx?.id === tx.id) {
                    return (
                      <tr key={tx.id} className="bg-neutral-50/50">
                        <td className="px-4 py-3 opacity-50">{tx.reference_code || '—'}</td>
                  ...
                  return (
                    <tr key={tx.id} className={`hover:bg-[var(--surface-muted)] transition ${tx.is_deleted ? 'opacity-40 grayscale line-through' : ''}`}>
                      <td className="px-4 py-3 font-medium text-neutral-800">{tx.reference_code || '—'}</td>
                      <td className="px-4 py-3">{formatDateShort(tx.date)}</td>
                      <td className="px-4 py-3">{tx.money_source_name || '—'}</td>
                      <td className={`px-4 py-3 text-right font-semibold ${tx.direction === 'in' ? 'text-emerald-700' : 'text-rose-700'}`}>
                        ₹{formatAmount(tx.amount)}{tx.direction === 'in' ? 'Cr' : 'Dr'}
                      </td>
                      <td className="py-3 pr-2 text-neutral-600">
                        <div>{tx.note || '—'}</div>
                        {(tx.lead_id || tx.category_name || tx.is_transfer || tx.is_overhead) && (
                          <div className="mt-1 text-xs text-neutral-500 space-y-0.5">
                            {tx.lead_id && (
                              <div>Lead: {tx.lead_number ? `#${tx.lead_number} · ` : ''}{tx.lead_name || 'Lead'}</div>
                            )}
                            {tx.category_name && <div>Category: {tx.category_name}</div>}
                            {tx.is_overhead && <div>Overhead</div>}
                            {tx.is_transfer && <div>Transfer</div>}
                          </div>
                        )}
                      </td>
                      <td className="px-4 py-3 text-right">
                        {!tx.is_deleted && !tx.is_transfer && (
                          <>
                        ...
                        {!tx.is_deleted && tx.is_transfer && tx.transfer_group_id && (
                          <Link className="text-brand-600 font-medium hover:text-brand-800 text-xs" href={`/admin/finance/transfers/${tx.transfer_group_id}`}>
                            View Transfer
                          </Link>
                        )}
                      </td>

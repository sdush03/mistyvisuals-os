...
export default function SalesLeadPage() {
  ...
  useEffect(() => {
    ...
    window.history.replaceState(null, '', `/leads/${id}/classic?${params.toString()}`)
  ...
  useEffect(() => {
    ...
    const params = new URLSearchParams(searchParams?.toString() || '')
    params.delete('edit')
    router.replace(`/leads/${id}/classic?${params.toString()}`, { scroll: false })
  ...
  const openTabSection = (
    tab: 'dashboard' | 'contact' | 'notes' | 'activity' | 'enrichment',
    targetId?: string
  ) => {
    setActiveTab(tab)
  ...
  const updateLeadStatus = async (
    ...
    if (pendingNegotiationPrompt) {
      setStatusChangedInfo({ message: `Status changed to Negotiation`, origin: statusChangeOrigin || 'lead' })
    } else {
      setStatusChangedInfo({ message: `Status changed to ${status}`, origin: statusChangeOrigin || 'lead' })
    }
  }
      ...
      <div className="mx-auto max-w-6xl px-4 md:px-6 pt-0 pb-8 space-y-8">
        ...
        <FollowUpActionPopup
          ...
          onSuccess={async (updated: FollowupUpdatedLead, meta?: FollowupSuccessMeta) => {
            ...
            if (updated?.auto_negotiation?.attempted && !updated?.auto_negotiation?.success) {
              ...
              setStatusChangedInfo({ message: 'Status changed to Negotiation', origin: 'lead' })
            }
            try {
        ...
        <div className="w-full flex flex-col md:flex-row md:items-center md:w-fit gap-1 md:rounded-full md:border border-[var(--border)] bg-white md:px-2 md:py-1 md:shadow-sm rounded-xl px-0 py-0 shadow-none border-0">
          ...
          <div className="md:hidden relative w-full mb-4">
            ...
            >
              {['dashboard', 'enrichment', 'contact', 'notes', 'activity', 'quotes'].map(tab => (
                <option key={tab} value={tab}>
                  {tab === 'enrichment' ? 'Details' : tab === 'notes' ? 'Notes' : tab.charAt(0).toUpperCase() + tab.slice(1)}
                </option>
              ))}
            </select>
          ...
          <div className="hidden md:flex items-center gap-1">
            {['dashboard', 'enrichment', 'contact', 'notes', 'activity', 'quotes'].map(tab => (
              ...
              >
                {tab === 'enrichment' ? 'Details' : tab.charAt(0).toUpperCase() + tab.slice(1)}
              </button>
            ))}
          </div>
        </div>

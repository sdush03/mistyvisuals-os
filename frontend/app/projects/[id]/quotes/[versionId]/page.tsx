...
const QuoteBuilderPage = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const projectPayload = await leadRes.json().catch(() => null)
        const leadData = projectPayload?.data?.project || null
  ...
  const handleApplyRevision = async () => {
     const teamMembers = (draft.pricingItems || []).filter((i: any) => i.itemType === 'TEAM_ROLE')
     const events = draft.events || []
     
     // Ensure every event has at least one crew member
     for (const ev of events) {
        const evCrew = teamMembers.filter((tm: any) => tm.eventId === ev.id)
        if (evCrew.length === 0) {
     ...
     const deliverables = (draft.pricingItems || []).filter((i: any) => i.itemType === 'DELIVERABLE')
     ...
     deliverables.forEach((d: any) => {
        const cat = (d as any).category || deliverablesCatalog.find((c: any) => c.id === d.catalogId)?.category || 'OTHER'
     ...
     try {
        ...
        // 3. Apply revision to project
        const res = await apiFetch(`/api/quote-versions/${versionId}/apply-project-revision`, { method: 'POST' })
        if (!res.ok) {
           const errData = await res.json()
           throw new Error(errData.error || 'Failed to apply revision')
        }

        alert('Revision successfully applied to project!')
        router.push(`/projects/${leadId}`)
     } catch (e: any) {
        alert(e.message)
     } finally {
      ...
      <div className="bg-white border-b border-neutral-200 px-8 py-5 sticky top-0 z-40 shadow-sm flex items-center justify-between">
         <div>
            <div className="flex items-center gap-3 mb-1">
               <Link href={`/projects/${leadId}`} className="text-xs font-semibold text-neutral-400 hover:text-neutral-900 transition flex items-center">
                  ← Back to Project
               </Link>
         ...
         <div className="flex items-center gap-3">
            ...
            {quoteStatus !== 'ACCEPTED' ? (
               <button disabled={approvalBusy} onClick={handleApplyRevision} className="px-5 py-2 bg-neutral-900 hover:bg-neutral-800 disabled:opacity-50 text-white rounded-full text-sm font-semibold transition shadow-sm flex items-center gap-2">
                  {approvalBusy ? (
                     <><div className="w-4 h-4 rounded-full border-2 border-white/30 border-t-white animate-spin" /> Applying Revision...</>
                  ) : (
                     <>📝 Apply Revision to Project</>
                  )}
               </button>
            ) : (
               <div className="px-4 py-2 bg-emerald-50 text-emerald-700 text-xs font-semibold rounded-lg border border-emerald-200 cursor-default">
                  Active Project Contract (Applied)
               </div>
            )}
         </div>
      </div>
